...
const WxcUtil = {
  ...
  isPlainObject (obj) {
    return WxcUtil._typeof(obj) === 'object';
  },
  isString (obj) {
  ...
  mergeDeep (target, ...sources) {
    ...
    if (WxcUtil.isObject(target) && WxcUtil.isObject(source)) {
      for (const key in source) {
        if (WxcUtil.isObject(source[key])) {
          if (!target[key]) {
            Object.assign(target, {
              [key]: {}
            });
          }
          WxcUtil.mergeDeep(target[key], source[key]);
        } else {
          Object.assign(target, { [key]: source[key] });
        }
      }
    }
    return WxcUtil.mergeDeep(target, ...sources);
  },
  appendProtocol (url) {
    if (/^\/\//.test(url)) {
      const {
  ...
  goToH5Page (jumpUrl, animated = false, callback = null) {
    ...
    const jumpUrlObj = new WxcUtil.UrlParser(jumpUrl, true);
    const url = WxcUtil.appendProtocol(jumpUrlObj.toString());
    Navigator.push({
      url: WxcUtil.encodeURLParams(url),
  ...
  env: {
    ...
    isIPhoneX () {
      ...
      return WxcUtil.env.isIOS() && deviceHeight === 2436;
    },
    isAndroid () {
      const { platform } = weex.config.env;
    ...
    isAliWeex () {
      return WxcUtil.env.isTmall() || WxcUtil.env.isTrip() || WxcUtil.env.isTaobao();
    },
    /**
    ...
    getPageHeight () {
      ...
      const navHeight = WxcUtil.env.isWeb() ? 0 : (WxcUtil.env.isIPhoneX() ? 176 : 132);
   ...
   * @example
   *
   * const { WxcUtil } = require('@ali/wx-bridge');
   * const { compareVersion } = WxcUtil;
