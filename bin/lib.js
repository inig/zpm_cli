...
const showAllCommands = function (withCopyRight) {
  ...
  console.log('\n - init\t\t添加模板'.data)
  console.log(
    ' \t\tzpm init [, project_template] [, project_name] [, project_path]'.data
  )
  console.log('\n - plugins\t添加组件命令'.data)
  console.log(
    ' \t\tzpm plugins add 组件1 [, 组件2]，添加组件，多个组件用空格区分，组件名支持 * （通配符，匹配一个或多个字符）'
      .data
  )
  ...
  console.log(
    ' \t\t输入 zpm sync help 或者 zpm sync ? 查看zpm sync完整命令'.data
  )
  // showSyncHelp(false);
  console.log('\n - help, ?\t显示所有命令'.data)
...
const showSyncHelp = function (withCopyRight) {
  ...
  console.log(
    '\n - @作者名\t同步指定作者的组件，多个作者以空格分隔，作者名前需要添加@符号'
      .data
  )
  console.log(' \t\tzpm sync @作者1 [, @作者2] ...'.data)
  console.log(
    ' \t\t作者名和组件名可以混合使用，作者名前带@符号，组件名前不带@符号: zpm sync @作者1 组件1 @作者2 组件2 ...'
      .warn
  )
...
const showZpmListActionHelp = function (withCopyRight) {
  ...
  console.log(
    '\t\t\tzpm ls [或list] author 或者 zpm ls [或list] authors\n'.data
  )
  console.log(' - plugin, plugins\t显示所有可用的组件'.data)
  console.log(
    '\t\t\tzpm ls [或list] plugin 或者 zpm ls [或list] plugins\n'.data
  )
  console.log(' - template, templates\t显示所有可用的模板'.data)
  console.log(
    '\t\t\tzpm ls [或list] template 或者 zpm ls [或list] template\n'.data
  )
...
const showNoPluginsTip = function (withCopyRight) {
  ...
  console.log(
    '\n       输入 "zpm sync 组件1 [, 组件2]" 同步指定的组件，多个组件用空格区分'
      .data
  )
  console.log(
    '\n       输入 "zpm sync @作者1 [, @作者2]" 同步指定作者的组件，多个作者用空格区分，作者名前带@符号'
      .data
  )
  console.log(
...
const pluginsCommandList = function (opts) {
  ...
  for (let i = 0; i < allPluginsConfig.length; i++) {
    ...
    if (isEmptyObj(allPluginsConfig[i].value)) {
      ...
      console.log(
        `- ${allPluginsConfig[i].name}  `.cyan,
        `${allPluginsConfig[i].value.description}`
      )
      console.log(
    ...
    if (i === allPluginsConfig.length - 1) {
      console.log(' ')
    }
  }
}
    ...
    if (p.indexOf('*') < 0) {
      ...
      plugins.forEach(function (plugin) {
        ...
        if (plugin.match(regStr)) {
          if (
            outPlugins.indexOf(plugin) < 0 &&
            legalPlugins.indexOf(plugin) < 0
          ) {
    ...
    fs.exists(realPath, function (exists) {
        ...
        try {
          shelljs.cp(
            ...
            `${path.join(
              __dirname,
              '.' + sep
            )}/plugins${sep}${plugin}${sep}index${pluginSuffix}`,
            `${realPath}${sep}${plugin}.vue`
          )
          let pkg = require(`${path.join(
            __dirname,
            '.' + sep
          )}/plugins${sep}${plugin}${sep}package.json`)
      ...
      if (successInstalled.length > 0) {
        console.log(
          '\n DONE '.successTag,
          '组件'.success,
          `${successInstalled.join('、')}`.success.bold.italic,
          '添加成功\n'.success
        )
    ...
    .then(() => {
      ...
      try {
        shelljs.cp(
          ...
          path.resolve(
            __dirname,
            `..${sep}zpm_plugins-master${sep}plugins${sep}*`
          ),
          pluginPath
        )
        setTimeout(() => {
          console.log('\n DONE '.successTag, '组件同步成功'.success)
          let stats = fs.readdirSync(
            path.resolve(__dirname, `..${sep}zpm_plugins-master${sep}plugins`)
          )
          if (stats) {
            if (stats.length > 0) {
              console.log('\n 同步的组件有'.grey)
...
const syncAllPluginsFromDei2 = async function syncAllPlugins (args) {
  ...
  let pluginsData = await request.post(
    '/Zpm/cli/listPlugins',
    qs.stringify(_searchConditions)
  )
  if (Number(pluginsData.data.status) === 200) {
    ...
    for (i; i < _allPlugins.length; i++) {
      let downloadedPluginData = await syncOnePluginFromDei2(
        _allPlugins[i].name,
        args.process
      )
...
const syncOnePlugin = function (plugin) {
  ...
  let downVuePromise = new Promise((resolve, reject) => {
    download(
      `https://raw.githubusercontent.com/lsliangshan/zpm_plugins/master/plugins/${plugin}/index.vue`,
      pluginPath,
      {}
    )
      .then(() => {
      ...
      .catch(err => {
        let vuePath = path.resolve(
          __dirname,
          '.' + sep + 'plugins' + sep + plugin + sep + 'index.vue'
        )
        fs.existsSync(vuePath) && fs.unlinkSync(vuePath)
        let pkgPath = path.resolve(
          __dirname,
          '.' + sep + 'plugins' + sep + plugin + sep + 'package.json'
        )
  ...
  let downPkgPromise = new Promise((resolve, reject) => {
    download(
      `https://raw.githubusercontent.com/lsliangshan/zpm_plugins/master/plugins/${plugin}/package.json`,
      pluginPath,
      {}
    )
      .then(() => {
